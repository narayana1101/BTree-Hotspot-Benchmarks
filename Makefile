CC = gcc-mp-5 
CXX = g++-mp-5 -std=gnu++0x
DEPSDIR := masstree/.deps
DEPCFLAGS = -MD -MF -MP #$(DEPSDIR)/$*.d
...
workload.o: workload.cpp microbench.h index.h util.h BTreeOLC/BTreeOLC.h BTreeOLC/BTreeOLC_child_layout.h ./pcm/pcm-memory.cpp ./pcm/pcm-numa.cpp ./papi_util.cpp # ./masstree/mtIndexAPI.hh ./BwTree/bwtree.h
	$(CXX) $(CFLAGS) -c -o workload.o workload.cpp

workload: skiplist-clean workload.o btree.o ./pcm/libPCM.a  #$(SL_OBJS) ./masstree/mtIndexAPI.a  bwtree.o artolc.o
	$(CXX) $(CFLAGS) -o workload workload.o btree.o ./pcm/libPCM.a $(MEMMGR) -lpthread -lm -ltbb # bwtree.o artolc.o  $(SL_OBJS) masstree/mtIndexAPI.a

#workload_string.o: workload_string.cpp microbench.h index.h util.h BTreeOLC/BTreeOLC.h skiplist-clean #./masstree/mtIndexAPI.hh ./BwTree/bwtree.h
...
#	$(CXX) $(CFLAGS) -o workload_string workload_string.o $(MEMMGR) -lpthread -lm -ltbb # bwtree.o artolc.o  $(SL_OBJS) masstree/mtIndexAPI.a
